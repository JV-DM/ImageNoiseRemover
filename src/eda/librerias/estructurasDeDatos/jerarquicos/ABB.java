...
public class ABB<E extends Comparable<E>> {
    ...
    protected NodoABB<E> construirEquilibrado(E[] v, int ini, int fin) {
        ...
        int medio= (ini + fin)/2;
        NodoABB<E> nodo = new NodoABB<E>(
                v[medio],
                construirEquilibrado(v, ini, medio -1),
                construirEquilibrado(v, medio+1, fin)
        );
    ...
    public String toStringPorNiveles() {
        ...
        Cua<NodoABB<E>> q = new ArrayCua<NodoABB<E>>();
        q.encuar(this.raiz);
        while (!q.esBuida()) {
            NodoABB<E> actual = q.desencuar();
            res.append(actual.dato.toString());
            res.append(", ");
            if (actual.izq != null) { q.encuar(actual.izq); }
            if (actual.der != null) { q.encuar(actual.der); }
        }
